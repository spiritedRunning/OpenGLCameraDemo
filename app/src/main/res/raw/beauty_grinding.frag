...
void main() {
    ...
    highp vec3 greenColor = vec3(1.0) - (vec3(1.0) - smoothColor) * (vec3(1.0) - centralColor);
    highp vec3 highLight = max(smoothColor, centralColor);
    highp vec3 lambency = 2.0 * centralColor * smoothColor + centralColor * centralColor - 2.0 * centralColor * centralColor * smoothColor;

    gl_FragColor = vec4(mix(centralColor, greenColor, alpha), 1.0);
}
